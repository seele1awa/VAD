...
FLOWNET_INPUT_WIDTH = {"ped2": 512 * 2, "avenue": 512 * 2, "shanghaitech": 1024, "cusco": 512*2}
FLOWNET_INPUT_HEIGHT = {"ped2": 384 * 2, "avenue": 384 * 2, "shanghaitech": 640, "cusco": 384*2}
...
def extracting_flows(dataset, dataset_name, of_save_dir):
    ...
    for idx, (batch, _) in tqdm(enumerate(dataset_loader), total=len(dataset)):
        ...
        cur_img_name = os.path.basename(cur_img_addr)

        # path to store flows
        video_of_path = os.path.join(of_save_dir, os.path.basename(os.path.dirname(cur_img_addr)))
