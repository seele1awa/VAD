...
def samples_extraction(dataset_root, dataset_name, mode, all_bboxes, save_dir):
    ...
    for idx in range(len(dataset)):
        ...
        # all the bboxes in current frame - 现在是7列的 [x1, y1, x2, y2, label, cx, cy]
...
if __name__ == '__main__':
    ...
    # 加载新的7列边界框数据 [x1, y1, x2, y2, label, cx, cy]
    all_bboxes = np.load(
    ...
    # 验证边界框数据的形状
    if len(all_bboxes) > 0 and len(all_bboxes[0]) > 0:
        print(f"Loaded bboxes with shape: {all_bboxes[0].shape}")
        print(f"Sample bbox: {all_bboxes[0][0] if len(all_bboxes[0]) > 0 else 'Empty'}")
