...
from utils.bbox_utils import extract_bbox_features
    ...
    for epoch in range(epoch_last, epochs + epoch_last):
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            for idx, train_data in tqdm(enumerate(dataloader),
                ...
                sample_frames, sample_ofs, bbox_data, _, _ = train_data
                sample_ofs = sample_ofs.to(device)
                sample_frames = sample_frames.to(device)

                # 从 bbox_data 中提取边界框特征
                bbox_features = extract_bbox_features(bbox_data)  # [B, 3]
                bbox_features = bbox_features.to(device)

                out = model(sample_frames, sample_ofs, bbox_features=bbox_features, mode="train")
    ...
    with torch.no_grad():
        ...
        for chunk_file_idx, chunk_file in enumerate(training_chunk_samples_files):
            ...
            for idx, data in tqdm(enumerate(dataloader),
                                  ...
                                  total=len(dataloader)):
                sample_frames, sample_ofs, bbox_data, _, _ = data
                sample_frames = sample_frames.to(device)
                sample_ofs = sample_ofs.to(device)

                # 提取边界框特征
                bbox_features = extract_bbox_features(bbox_data)
                bbox_features = bbox_features.to(device)

                out = model(sample_frames, sample_ofs, bbox_features=bbox_features, mode="test")
