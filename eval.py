...
METADATA = {
    ...
    "cusco": {
        "testing_video_num": 7,
        "testing_frames_cnt": [853, 2781, 714, 4533, 3955, 3395, 3054],
    },
...
def evaluate(config, ckpt_path, testing_chunked_samples_file, training_stats_path, suffix):
    ...
    for test_data in tqdm(dataloader_test, desc="Eval: ", total=len(dataloader_test)):
        ...
        # 提取边界框特征
        bbox_features = extract_bbox_features(bbox_test)
        bbox_features = bbox_features.to(device)

        out_test = model(sample_frames_test, sample_ofs_test, bbox_features=bbox_features, mode="test")
